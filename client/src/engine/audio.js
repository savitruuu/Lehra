import { TAAL_DATA, PITCH_MAP } from "./taalData.js";
import { InstrumentSampler } from "./instruments.js";
import { TablaSampler } from "./tabla.js";
...
/**
 * The reference Sa in Hz for a given scale and cent offset.
 *
 * Pulled out of the engine method so the UI can ask the same question about a
 * scale it has not committed yet. React writes the engine from an effect, which
 * runs after the render that changed it - so a readout calling the method would
 * show the previous scale's frequency for one frame on every change.
 */
function saFrequencyFor(pitch, cents) {
  let base = PITCH_MAP[pitch] || 277.18; // Default C#
  // The top five scales are taken an octave down - PITCH_MAP is written in
  // octave 4, which is higher than these sit in practice.
  if (["G", "G#", "A", "A#", "B"].includes(pitch)) {
    base = base / 2;
  }
  return base * Math.pow(2, (cents || 0) / 1200);
}
...
class LehraAudioEngine {
  ...
  initStretchWorker() {
    ...
    try {
      // Bundled by Vite from the module worker beside this file, rather than
      // fetched as a loose script off the document root the way the pre-build
      // app did - so the worker and the DSP it uses stay one source of truth.
      this.stretchWorker = new Worker(
        new URL("./tanpuraWorker.js", import.meta.url),
        { type: "module" }
      );
   ...
   */
  saFrequency() {
    return saFrequencyFor(this.pitch, this.pitchCents);
  }
  ...
  getResolvedNote(index, notes) {
    ...
    if (window._activeRaagKey === "hemant" && this.bpm >= 190) {
      const drutNotes = [12, null, 12, null, 12, null, 16, 17, 14, null, 12, null, 11, 12, 9, 11];
      return drutNotes[index % drutNotes.length];
    }
